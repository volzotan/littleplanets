...
@dataclass
class Mesh:
    points: list[np.array] | np.ndarray
    faces: list[np.array] | np.ndarray  
    colors: list[np.array] | np.ndarray | None = None
    centers: list[np.array] | np.ndarray | None = None
    normals: list[np.array] | np.ndarray | None = None
    field_vectors: list[np.array] | np.ndarray | None = None
    field_elevation_vectors: list[np.array] | np.ndarray | None = None
    
    # Internal flag to track if we're using numpy arrays (for efficiency)
    ...
    def to_numpy(self):
        """Convert internal representation to numpy arrays for faster processing"""
        if not self._use_numpy and isinstance(self.points, list):
            self.points = np.array(self.points, dtype=np.float32) if self.points else np.empty((0, 3), dtype=np.float32)
            self.faces = np.array(self.faces, dtype=np.int32) if self.faces else np.empty((0, 3), dtype=np.int32)
    ...
    def to_lists(self):
        """Convert back to list representation for compatibility"""
        if self._use_numpy and isinstance(self.points, np.ndarray):
            self.points = [self.points[i] for i in range(len(self.points))]
            self.faces = [self.faces[i] for i in range(len(self.faces))]
            if self.colors is not None:
                self.colors = [self.colors[i] for i in range(len(self.colors))]
...
def write_ply(m: Mesh, filename: Path, color_vertices=False) -> None:
    # Handle both numpy arrays and lists efficiently
    if isinstance(m.points, np.ndarray):
        points_data = m.points
        faces_data = m.faces
        colors_data = m.colors
    else:
        points_data = np.array(m.points)
        faces_data = np.array(m.faces)
        colors_data = np.array(m.colors) if m.colors else None
    ...
    if not color_vertices:  # faces
        with open(filename, "w") as f:
            data = f"""
                    ply
                    format ascii 1.0
                    element vertex {len(points_data)}
                    property float x
                    property float y
                    property float z
                    element face {len(faces_data)}
            ...
            # Write vertices (vectorized formatting)
            for i in range(len(points_data)):
                p0, p1, p2 = points_data[i]
                f.write(f"{p0:.4f} {p1:.4f} {p2:.4f}\n")

            # Write faces with colors (vectorized access)
            for i in range(len(faces_data)):
                i0, i1, i2 = faces_data[i]
                c0, c1, c2 = colors_data[i] if colors_data is not None else [255, 255, 255]
        ...
        with open(filename, "w") as f:
            ...
            # Write colored vertices (vectorized access)
            for i in range(len(points_data)):
                p0, p1, p2 = points_data[i]
                c0, c1, c2 = colors_data[i] if colors_data is not None else [255, 255, 255]
                f.write(f"{p0:.4f} {p1:.4f} {p2:.4f} {c0:d} {c1:d} {c2:d}\n")

            # Write faces (vectorized access)
            for i in range(len(faces_data)):
                i0, i1, i2 = faces_data[i]
...
def subdivide(m: Mesh, n: int) -> Mesh:
    ...
    # Convert to numpy format for efficient processing
    m_numpy = m.to_numpy()
    points_array = m_numpy.points
    faces_array = m_numpy.faces
    
    # Process all subdivision levels iteratively
    for level in range(n):
        num_faces = len(faces_array)
        num_points = len(points_array)
        
        # Pre-compute all edges for all faces vectorized
        # Each face has 3 edges: (v0,v1), (v1,v2), (v2,v0)
        all_edges = np.empty((num_faces * 3, 2), dtype=np.int32)
        all_edges[0::3] = np.stack([faces_array[:, 0], faces_array[:, 1]], axis=1)  # edge 0-1
        all_edges[1::3] = np.stack([faces_array[:, 1], faces_array[:, 2]], axis=1)  # edge 1-2
        all_edges[2::3] = np.stack([faces_array[:, 2], faces_array[:, 0]], axis=1)  # edge 2-0
        ...
        # Calculate midpoints for all unique edges at once (vectorized)
        edge_midpoints = (points_array[unique_edges[:, 0]] + points_array[unique_edges[:, 1]]) * 0.5
        
        # Create new points array
        new_points = np.vstack([points_array, edge_midpoints])
        
        # Map edge indices to new point indices
        edge_to_new_point = num_points + np.arange(num_unique_edges)
        
        # For each face, get the 3 midpoint indices
        midpoint_indices = edge_to_new_point[edge_inverse].reshape(num_faces, 3)
        
        # Create new faces vectorized
        new_faces = np.empty((num_faces * 4, 3), dtype=np.int32)
        
        # Original vertices
        v0, v1, v2 = faces_array[:, 0], faces_array[:, 1], faces_array[:, 2]
        # Midpoint indices
        m01, m12, m20 = midpoint_indices[:, 0], midpoint_indices[:, 1], midpoint_indices[:, 2]
        
        # Create 4 triangles per original face
    ...
    # Return optimized mesh (keep in numpy format)
    result = Mesh(points_array, faces_array)
    result._use_numpy = True
    return result
...
def project(m: Mesh, raster: np.ndarray | None, scale: float = 0.1) -> Mesh:
    # Ensure we're working with numpy arrays for efficiency
    m_numpy = m.to_numpy()
    points_array = m_numpy.points

    # Vectorized cartesian to spherical conversion
    r_vec = np.linalg.norm(points_array, axis=1)
    lat_vec = np.arccos(points_array[:, 2] / r_vec)
    lon_vec = np.arctan2(points_array[:, 1], points_array[:, 0])

    # Initialize radii
    r_new = np.ones(len(points_array))

    if raster is not None:
        # Vectorized raster mapping
        height, width = raster.shape
        y_indices = ((lat_vec / math.pi) * height).astype(int)
        x_indices = ((lon_vec / math.tau) * width).astype(int)

        # Clamp latitude indices only (longitude can wrap around with negative indexing)
        y_indices = np.clip(y_indices, 0, height - 1)

        r_new += raster[y_indices, x_indices] * scale

    # Vectorized spherical to cartesian conversion and in-place update
    points_array[:, 0] = r_new * np.sin(lat_vec) * np.cos(lon_vec)
    points_array[:, 1] = r_new * np.sin(lat_vec) * np.sin(lon_vec)
    points_array[:, 2] = r_new * np.cos(lat_vec)
...
def add_color(m: Mesh, raster: np.ndarray, color_vertices=False) -> Mesh:
    ...
    if not color_vertices:
        # Get first vertex of each face (more efficient with numpy indexing)
        points_array = m_numpy.points[m_numpy.faces[:, 0]]
    else:
        points_array = m_numpy.points

    # Vectorized cartesian to spherical conversion
    d_vec = np.linalg.norm(points_array, axis=1)
    lat_vec = np.arccos(points_array[:, 2] / d_vec)
    lon_vec = np.arctan2(points_array[:, 1], points_array[:, 0])
    ...
    # Extract colors for all points at once and keep as numpy array
    colors = raster[y_indices, x_indices].astype(np.uint8)
    m_numpy.colors = colors
...
def add_seedpoints(m: Mesh, num: int) -> Mesh:
    ...
    seedpoints_array = np.array(seedpoints)

    # Use vectorized computation for face centers
    points_array = np.array(m.points)
    faces_array = np.array(m.faces)
    centers = np.mean(points_array[faces_array], axis=1)

    # Use cKDTree for much faster nearest neighbor search
    tree = cKDTree(centers)

    # Query all seedpoints at once
    _, nearest_indices = tree.query(seedpoints_array)

    # Update colors for nearest faces
    for nearest_triangle_index in nearest_indices:
...
def _compute_normals(m: Mesh) -> tuple[np.ndarray, np.ndarray]:
    ...
    v0 = points_array[faces_array[:, 0]]
    v1 = points_array[faces_array[:, 1]]
    v2 = points_array[faces_array[:, 2]]
...
def _compute_intersections(m: Mesh, centers: np.ndarray, normals: np.ndarray, axis: np.array, tol=1e-6) -> np.ndarray:
    ...
    # Vectorized line-plane intersection computation
    denom = np.dot(normals, axis)

    # Handle parallel lines
    parallel_mask = np.abs(denom) < tol
    t = np.zeros(len(centers))

    # Compute t for non-parallel cases
    non_parallel_mask = ~parallel_mask
    t[non_parallel_mask] = -np.dot(normals[non_parallel_mask], centers[non_parallel_mask] - line_point) / denom[non_parallel_mask]

    # Compute intersections
    intersections = line_point + t[:, np.newaxis] * axis

    # Handle parallel cases (return line_point or axis based on original logic)
    intersections[parallel_mask] = line_point
...
def field_vectors_to_image(m: Mesh, num: int = 360) -> np.ndarray:
    ...
    # Vectorize the coordinate generation
    ...
    latr_grid = np.deg2rad(lat_grid)
    lonr_grid = np.deg2rad(lon_grid)

    # Vectorized spherical to cartesian conversion
    x_grid = np.cos(latr_grid) * np.cos(lonr_grid)
    y_grid = np.cos(latr_grid) * np.sin(lonr_grid)
    z_grid = np.sin(latr_grid)

    # Stack coordinates for batch query
    coords = np.stack([x_grid.ravel(), y_grid.ravel(), z_grid.ravel()], axis=1)

    # Query all coordinates at once
    _, nearest_indices = tree.query(coords)

    # Convert field_vectors to numpy array for faster indexing
    field_vectors_array = np.array(m.field_vectors)

    # Get directions and reshape to image
    directions = field_vectors_array[nearest_indices]
    image = directions.reshape(num, num, 3)
